...
> Reglas de desarrollo, arquitectura, colores, nomenclatura y diseño → `docs/historico/rules.md`
...
- Ver: docs/historico/fases/fase-1-sprint-1-registro.md (registro — historial completo)
- Ver: docs/registro.html (documentación técnica de la vista /registro)
- Ver: docs/historico/fases/fase-1-sprint-1-landing.md (landing — historial completo)
- Ver: docs/historico/fases/fase-1-sprint-1-carrousel-completados.md (carrusel con imágenes reales — ✅ completado)
- Ver: docs/historico/desarrollo/landingPage.md (documentación técnica de todos los componentes)
...
- /docs → documentación y contexto
  - /docs/historico/desarrollo → documentación técnica de cada módulo
  - /docs/historico/fases → sprints y tareas pendientes
  - /docs/historico/cambios → registro de cambios por capa (ui, api, domain)

## Tareas Pendientes de Fase 0

- Ver: docs/historico/fases/fase-0-pendientes.md
...
- [SETUP-005] Paquetes deprecated (eslint@8, next-pwa@5) → aplazado a Fase 6, ver docs/historico/fases/tech-debt.md

## Carrusel — ✅ Completado (Sprint 3)

- Ver: docs/historico/fases/fase-1-sprint-1-carrousel-completados.md
...
Pendiente documentado en: docs/historico/fases/fase-0-pendientes.md [SETUP-006] [SETUP-007]
...
- ✅ [NUTR-001] Completado — calcularMacros() en nutritionService.ts llama a Edamam Nutrition Analysis API al publicar una receta. recetaRepository.crear() sustituye macros ceros por el resultado real. Sin claves devuelve ceros sin error. Requiere EDAMAM_APP_ID + EDAMAM_APP_KEY (100/mes gratis).
- ✅ [NUTR-002] Completado — USDA FoodData Central como fallback cuando Edamam falla. calcularMacrosUSDA() busca cada ingrediente individualmente, escala por cantidad/unidad, acumula macros. Flujo: Edamam → USDA → ceros. Requiere USDA_API_KEY (api.data.gov, gratuita).
- ✅ [INGR-BE-001] Completado — ingredientesService.ts (BE) conectado a Open Food Facts suggest.pl (en español, sin key, gratuito). GET /api/ingredientes/buscar?q= registrado. Frontend conectado via apiClient en lugar de devolver [].
- ✅ [INGR-FE-002] Completado — ingredientes.ts ampliado de 239 a 991 ingredientes en 18 categorías (nueva: preparados). Fuentes: BEDCA, Mercadona/Carrefour, Open Food Facts. 0 errores tsc.
- ✅ [DISC-SCORE-001] Completado — Algoritmo scoring híbrido en tab Recientes de /discover. score = (likes×2 + comentarios×3) × decay + followBoost + prefBoost. calcularScoreFeed() + obtenerPreferenciasUsuario() en recetaRepository. useDiscover.ts mapea tab recientes → sort:score. 0 errores tsc.
- ✅ [PEXELS-CASCADE-001] Completado — buscarFotoPexelsCascada() en imagenService.ts. Cascada: tipo de plato (sopa→soup, pasta→pasta dish…) → ingrediente principal (pollo→chicken…) → categoría genérica. Nunca manda el título crudo a Pexels. Sustituye buscarFotoPexels() en recetaRepository.crear() y recetasService.obtenerFotoPreview(). 0 errores tsc.
- ✅ [SEC-IA-001] Completado — Auditoría endpoints IA: 7 vulnerabilidades corregidas. Prompt injection sanitizada. Rate limit por usuario (chat 30/min, generar 5/min, ticket 5/min). Límites chat (50 msgs, 2000 chars). Jailbreak por rol bloqueado. imagenBase64 validado. /foto-preview requiere auth. Schema validation en respuestas Gemini. Nuevo middleware rateLimitIA.ts. 0 errores tsc.
- ✅ [GEMINI-CACHE-001] Completado — 3 cachés en memoria en chatService.ts: (1) contexto usuario TTL 5min evita 2 queries MongoDB por mensaje; (2) singleton GoogleGenerativeAI reutilizado entre requests; (3) receta generada TTL 2min protege doble-submit. invalidarContextoUsuario() exportada. 0 cambios schema, 0 deps nuevas. 0 errores tsc.
- ✅ [CHAT-HIST-001] Completado — Historial de chats persistente. chatStore.ts reescrito con zustand persist (clave 'cookr-chat', solo historial via partialize, max 20 entradas). nuevaConversacion() archiva la sesión actual; restaurarConversacion(id) y eliminarConversacion(id). SheetHistorialChat.tsx nuevo: side=right, botón guardar+nueva, lista con fecha relativa y papelera por hover. Fix z-index: sheet.tsx SheetOverlay+SheetContent de z-50 a z-[70] (el contenedor del chat usa z-[60]).
- ✅ [CHAT-IMG-001] Completado — Adjuntar imagen al chat IA con Gemini Vision multimodal. FE: barraInputChat.tsx con input[file] oculto, FileReader a base64, preview thumbnail con X, límite 4 MB. BurbujaUsuario muestra la imagen. Al guardar en historial se elimina la imagen para no saturar localStorage. BE: chat.routes.ts acepta imagenBase64 opcional (valida data:image/, ≤7 MB). responderChat() envía inlineData a Gemini cuando hay imagen. Si no hay texto, prompt por defecto "¿Qué ves en esta imagen? Ayúdame en el contexto culinario." 0 errores tsc.
