...
- [x] **Arreglar `middlewares/rateLimitIA.ts`.** Hecho en la Fase 1 (purga de ventanas caducadas sin timers) y **sustituido en la Fase 4**: se reescribió sobre `express-rate-limit` con el store compartido, así que ya no se pierde en cada redeploy si hay Redis configurado.
...
- [x] **Rate limit con Redis.** Hecho (17/07/2026). `lib/rateLimitStore.ts` da la fábrica común: **store de Upstash si `UPSTASH_REDIS_URL` y `UPSTASH_REDIS_TOKEN` están definidas, `MemoryStore` si no.** El cliente REST vive en `lib/redis.ts` (fallback defensivo: si las vars están mal, avisa y sigue en memoria). El store propio para `express-rate-limit` es `lib/upstashRateLimitStore.ts` (`INCR` + `PEXPIRE` en el primer hit + `PTTL`), con test unitario contra un Redis falso (`tests/upstashStore.test.ts`, 5 casos). Los **dos** limitadores pasan por ahí: `rateLimitAuth.ts` (por IP) y `rateLimitIA.ts`, que se reescribió sobre `express-rate-limit` para dejar de perder las ventanas en cada redeploy (era lo que anotaba la Fase 1). Sin las vars, comportamiento idéntico a antes y tests en verde; con ellas, sobrevive a redeploys. **Lo verifica el autor en producción** (crear database en Upstash, poner las vars en Render); en local no se puede porque el guard del E2E prohíbe credenciales reales.
