        ...
        {pasos.map((paso, i) => (
          <Fragment key={i}>
            ...
            <li className={`flex gap-4 py-5 transition-all duration-300 ${i === pasoResaltado ? 'bg-brand/5 -mx-2 px-2 rounded-2xl ring-1 ring-brand/30' : ''}`}>
              <div className={`shrink-0 h-8 w-8 flex items-center justify-center rounded-full font-black text-sm border transition-colors ${i === pasoResaltado ? 'bg-brand text-brand-foreground border-brand' : 'bg-[var(--brand-subtle)] text-brand border-brand/20'}`}>
                {i + 1}
              </div>
